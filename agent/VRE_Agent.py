...
class cwlAgent(Agent):
    """
    This class define CWL agent.
    """
    ...
    def __init__(self, configuration=None):
        ...
        :param configuration: A dictionary containing parameters that define how the operation should be carried out,
            which are specific to CWL agent.
    ...
    def run(self, input_files, input_metadata, output_files, output_metadata):
        """
        The main function to run the CWL agent.
        ...
        try:
            ...
            if len(self.execution_provenance) != 0:
                # Create and validate the output file/s from agent execution
                self.cwl_wf.createOutputsFiles(output_files, output_metadata, self.execution_provenance,
                                               self.execution_path)

                # Remove agent execution intermediate files
            ...
            errstr = "CWL agent execution failed. See logs."
            logger.fatal(errstr)
            raise Exception(errstr)
        ...
        try:
            ...
            if os.path.isfile(cwl_wf_yaml_filename):
                ...
                if rc is not None and rc != 0:
                    logger.progress("Something went wrong inside the cwlagent execution. See logs", status="WARNING")
                else:
                    logger.progress("cwlagent execution finished successfully", status="FINISHED")

                # Save processed outputs from cwlagent execution
                output, error = process.communicate()
                self.execution_provenance = json.loads(output)

                # Create and validate RO-crate from cwlagent execution provenance
            ...
            errstr = "cwlagent execution failed. See logs."
            logger.error(errstr)
            if rc is not None:
